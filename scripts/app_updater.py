        ...
        if ip_addr != "" and app_name != "" and hw_name != "":
            ...
            if not is_bootloader:
                ...
                if firmware_file_name == None:
                    print(f"HW '{hw_name}' does not support an update of application")

            if firmware_file_name != None:
                #upload firmware.bin to the device
                print(f"Uploading {firmware_file_name} to {ip_addr}")
                upload_file(ip_addr, firmware_file_name, 'firmware.bin')
            else:
                print(f"File firmware.bin not specifed for {app_name}")
...
def main():
    ...
    if args.uio:
        ...
        if not os.path.exists(UIO_Path):
            raise FileNotFoundError(UIO_Path)
    if args.elm:
        print("elm:", args.elm)
        ELM_Path = args.elm
    ...
    if args.ifmux:
        ...
        if not os.path.exists(IfMux_Path):
            raise FileNotFoundError(IfMux_Path)
    if args.rdo:
        print("rdo:", args.rdo)
        RDO_Path = args.rdo
        if not os.path.exists(RDO_Path):
            raise FileNotFoundError(RDO_Path)
