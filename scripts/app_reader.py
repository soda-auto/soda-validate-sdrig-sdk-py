...
def print_devices():
    ...
    for device in devices:
        ...
        if "MODULE_INFO_BOOT" in devices[device]:
            module_info_boot_msg = devices[device]['MODULE_INFO_BOOT']
            if "module_app_fw_name_1" in module_info_boot_msg:
                fw_name_1 = module_info_boot_msg['module_app_fw_name_1'].to_bytes(8, 'little').decode('utf-8')
                fw_name_2 = module_info_boot_msg['module_app_fw_name_2'].to_bytes(8, 'little').decode('utf-8')
                fw_name_3 = module_info_boot_msg['module_app_fw_name_3'].to_bytes(8, 'little').decode('utf-8')
                fw_name = fw_name_1 + fw_name_2 + fw_name_3
                bl_name = fw_name.rstrip("\x00")

                bl_version = f"{module_info_boot_msg['module_app_ver_gen']}." \
                                f"{module_info_boot_msg['module_app_ver_major']}." \
                                f"{module_info_boot_msg['module_app_ver_minor']}." \
                                f"{module_info_boot_msg['module_app_ver_fix']}." \
                                f"{module_info_boot_msg['module_app_ver_build']}"
                bl_version = f"{bl_version} {module_info_boot_msg['module_app_target']}"

                bl_build_date = f"{module_info_boot_msg['module_app_build_day']:02d}/" \
                                    f"{module_info_boot_msg['module_app_build_month']:02d}/" \
                                    f"{module_info_boot_msg['module_app_build_year']:04d} " \
                                    f"{module_info_boot_msg['module_app_build_hour']:02d}:" \
                                    f"{module_info_boot_msg['module_app_build_min']:02d}"
                bl_crc = f"{module_info_boot_msg['module_app_crc']:08X}"
        ...
        if ip_addr != "" and app_name != "" and hw_name != "":
            ...
            if bl_name != "" and bl_version != "" and bl_build_date != "":
                print(f"Device BL Name| {bl_name} | {bl_version} | {bl_build_date} | {bl_crc}")
...
def create_avtp_acf_can_frame(vlan_id: int = 0):
    ...
    acf_can_data = b'\x3f\x00\x00\x00\x00\x00\x00\x00'
...
def main():
    global devices
    parser = argparse.ArgumentParser(description="Firmware updater for SDRig")
